...
console.log(FunctionDeclaration); // FunctionDeclaration() {}
console.log(ClassDeclaration);    // ReferenceError: ClassDeclaration is not defined复制代码
```
...
console.log(PersonName);    // ReferenceError: PersonName is not defined复制代码
```
...
1. **实例化**
   ...
   let v = new Vegetable();
   console.log(v.color);  // orange复制代码
   ```
   ...
   console.log(p2 instanceof Person);  // false复制代码
   ```
   ...
   let a = Animal();
   // TypeError: class constructor Animal cannot be invoked without 'new'复制代码
   ```
   ...
   // 使用对类构造函数的引用创建一个新实例
   let p2 = new p1.constructor();复制代码
   ```
...
2. **把类当成特殊函数**
   ...
   console.log(typeof Person);  // function复制代码
   ```
   ...
   console.log(p instanceof Person); // true复制代码
   ```
   ...
   console.log(p2 instanceof Person.constructor);  // true复制代码
   ```
   ...
   let foo = createInstance(classList[0], 3141);  // instance 3141复制代码
   ```
...
2. **原型方法与访问器**
   ...
   p.locate();                 // instance
   Person.prototype.locate();  // prototype复制代码
   ```
   ...
   // Uncaught SyntaxError: Unexpected token复制代码
   ```
   ...
   p.computedKey();  // invoked computedKey复制代码
   ```
...
3. **静态类方法**
   ...
   Person.locate();            // class, class Person {}复制代码
   ```
   ...
   console.log(Person.create()); // Person { age_: ... }复制代码
   ```
...
5. **迭代器与生成器方法**
   ...
   console.log(nicknameIter.next().value);  // J-Dog复制代码
   ```
...
1. **继承基础**
   ...
   console.log(e instanceof Person);    // true复制代码
   ```
   ...
   Vehicle.identifyClass('vehicle'); // vehicle, class Vehicle {}复制代码
   ```
...
2. **构造函数、`HomeObject`和`super()`**
   ...
   - `super()`的行为如同调用构造函数，如果需要给父类构造函数传参，则需要手动传入。
     ...
     console.log(new Bus('1337H4X')); // Bus { licensePlate: '1337H4X' }复制代码
     ```
   ...
   - 在类构造函数中，不能在调用`super()`之前引用`this`。
     ...
     // before accessing 'this' or returning from derived constructor复制代码
     ```
...
3. **抽象基类**
   ...
   // Error: Vehicle cannot be directly instantiated复制代码
   ```
   ...
   new Bus(); // success!
   new Van(); // Error: Inheriting class must define foo()复制代码
   ```
...
4. **继承内置类型**
   ...
   console.log(a2 instanceof SuperArray);  // true复制代码
   ```
   ...
   console.log(a2);  // [1, 3, 5]
   console.log(a1 instanceof SuperArray);  // true
   console.log(a2 instanceof SuperArray);  // false复制代码
   ```
