...
6. **模板字面量标签函数**
   ...
   console.log(untaggedResult);  // "6 + 9 = 15"
   console.log(taggedResult);    // "6 + 9 = 15"复制代码
   ```
...
2. **一元加和减**
   ...
   **一元加和减操作符**对大多数开发者来说并不陌生，它们在ECMAScript中跟在高中数学中的用途一样。一元加由一个加号（`+`）表示，放在变量前头，对数值没有任何影响：
   ...
   let num = 25;
   num = +num;
   console.log(num); // 25复制代码
   ```

   如果将一元加应用到非数值，则会执行与使用`Number()`转型函数一样的类型转换：布尔值`false`和`true`转换为0和1，字符串根据特殊规则进行解析，对象会调用它们的`valueOf()`和/或`toString()`方法以得到可以转换的值。
...
```
0000  0000  0000  0000  0000  0000  0001  0010复制代码
```

然后，计算一补数，即反转每一位的二进制值：

```
1111  1111  1111  1111  1111  1111  1110  1101复制代码
```
...
1111  1111  1111  1111  1111  1111  1110  1101
                        1
----------------------------------------------
1111  1111  1111  1111  1111  1111  1110  1110复制代码
```
...
```
let num = -18;
console.log(num.toString(2)); // "-10010"复制代码
```
...
5. **左移**
   ...
   let newValue = oldValue << 5;  // 等于二进制1000000，即十进制64复制代码
   ```
...
6. **有符号右移**
   ...
   let newValue = oldValue >> 5;  // 等于二进制10，即十进制2复制代码
   ```
...
7. **无符号右移**
   ...
   let newValue = oldValue >>> 5;   // 等于十进制134217726复制代码
   ```
...
2. **逻辑与**
   ...
   console.log(result); // 不会执行这一行复制代码
   ```
...
1. **加法操作符**
   ...
   console.log(message);  // "The sum of 5 and 10 is 510"复制代码
   ```
...
2. **减法操作符**
   ...
   let result6 = 5 - null; // null被转换为0，所以结果是5复制代码
   ```
...
2. **全等和不全等**
   ...
   let result2 = ("55" === 55);  // false，不相等，因为数据类型不同复制代码
   ```
   ...
   let result2 = ("55" !== 55); // true，不相等，因为数据类型不同复制代码
   ```
...
```
let max = (num1 > num2) ? num1 : num2;复制代码
```
...
```
for (property in expression) statement复制代码
```
...
```
sayHi("Nicholas", "how are you today?");复制代码
```
