...
2. **观察字符数据**
   ...
   `MutationObserver`可以观察文本节点（如`Text`、`Comment`或`ProcessingInstruction`节点）中字符的添加、删除和修改。要为字符数据注册回调，需要在`MutationObserverInit`对象中将`characterData`属性设置为`true`，如下所示：**2**
...
**2**设置元素文本内容的标准方式是`textContent`属性。`Element`类也定义了`innerText`属性，与`textContent`类似。但`innerText`的定义不严谨，浏览器间的实现也存在兼容性问题，因此不建议再使用了。——译者注
